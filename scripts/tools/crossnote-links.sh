...
# 已知软肋：编辑器原子保存（写临时文件再 rename）换 inode → 硬链变陈旧
# ——内容分歧时重跑本脚本重建即可。clone 后或链接损坏同样跑一遍。
set -euo pipefail
cd "$(dirname "$0")/../.."
...
for f in parser.js style.less config.js head.html pseudocode-runtime.js theme-toggle.js; do
  [[ -f $SRC/$f ]] || {
