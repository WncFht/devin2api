...
# 用法: scripts/deploy-linux.sh [--release <tag|latest>] [--no-restart] [--check]
# 参数语义与 macOS 版一致（见 deploy.sh 头部注释）。
#
...
# 未加载时自动生成并 enable --now，因此首装与升级同一条命令。
#
...
RUNNING="$(wait_healthz_version "${HEALTH_URL}" "${VERSION}" 90)" || {
	echo "healthz 未出现新版本 (last=${RUNNING}); check ${RUNTIME}/logs/stderr.log" >&2
	exit 1
}
