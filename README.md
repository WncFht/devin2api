...
| `devin.accounts`                                 | Upstream account pool entries `{name, token, credentials_file, priority, max_rpm}` — `token`/`credentials_file` at least one; `priority` orders lane picks (0 = default), `max_rpm` overrides the per-account rate cap; empty list = legal empty pool (manage accounts live from `/web/accounts.html`) | No — `/v1` returns `unavailable` until an account exists                          |
