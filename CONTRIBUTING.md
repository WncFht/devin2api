...
- `POST /v1/responses` — subset of the OpenAI Responses API: `input` (string or items: `message`, `function_call`, `function_call_output`, `custom_tool_call`, `custom_tool_call_output`, `reasoning`), `instructions`, `tools`, `stream`, `max_output_tokens`, `temperature`, `top_p`, `tool_choice`, `parallel_tool_calls`, `previous_response_id`, `prompt_cache_key`, `user`. `content` parts: `input_text`, `output_text`, `text`, `input_image` (base64 data URL), `input_file`/`file` (base64 `file_data` or `file_url`; `file_id` rejected), `video`/`video_url`/`input_video`. `tools` entries may also be `custom` (freeform), `namespace` (flattened sub-tools), or `web_search` (server-managed — executed via the upstream search RPC). `GET /v1/responses` upgrades to the OpenAI Responses WebSocket transport (`responses_websockets=2026-02-06`), mapping each SSE event to one text frame.
- `POST /v1/chat/completions` — OpenAI Chat: `messages`, `tools`, `tool_choice`, `stream`/`stream_options.include_usage`, `max_tokens`/`max_completion_tokens`, `temperature`, `top_p`, `top_k`, `seed`, `stop`, `parallel_tool_calls`, `prompt_cache_key`, `user`, `n`, plus legacy `functions`/`function_call`. Content parts take the same image/document/video forms as Responses. Unconsumed top-level fields (`response_format`, `reasoning_effort`, `store`, …) are recorded as dropped, not silently ignored.
- `POST /v1/messages` — Anthropic Messages: `system`, `messages` (text / `image` / `document`/`file` / `video`/`video_url` / `tool_use` / `tool_result` / `server_tool_use` / `server_tool_result` / `thinking`+`signature` / `redacted_thinking` blocks), `tools`, `tool_choice`, `max_tokens`, `stream`, `temperature`, `top_p`, `top_k`, `stop_sequences`, `metadata.user_id` (used as the session key for cache affinity). Server-tool declarations (`web_search_*`, `web_fetch_*`, `code_execution_*`, …) have no upstream counterpart and are recorded as dropped — except a request whose tools are all `web_search_*`, which short-circuits into a single managed search before `GetChatMessage`. The top-level `thinking` param has no upstream counterpart and is also recorded as dropped.
- `GET /v1/models` + `GET /v1/models/{model}` — upstream model list with capability flags (`context_tokens`, `max_output_tokens`, `supports_tool_calls`, `supports_parallel_tool_calls`, `supports_thinking`, `preserve_thinking`, `supports_images`, `supports_documents`, `supports_video`).
