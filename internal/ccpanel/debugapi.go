...
func (h *Handler) adminAPIIndex(w http.ResponseWriter, r *http.Request) {
	respondOK(w, map[string]any{
		...
		"endpoints": []map[string]string{
			...
			{"method": "GET", "path": "/admin/accounts", "description": "号池账号聚合视图：source(config|panel|tombstoned)+credential+disabled+token_sha+priority/max_rpm/notes+lane/gate/warm 快照+inflight+quota 摘要+usage{rpm_now,tps_now,ttfb_avg,ttfb_p50,ttfb_p90,cache_rate,today{requests,success_rate,tokens}}"},
			{"method": "POST", "path": "/admin/accounts", "description": "建号 {name, token?|credentials_file?|credentials_content?, disabled?, verify?, priority?, max_rpm?, notes?}；verify 先探测凭据失败 400 不建行；file 与 content 互斥；整表校验失败 400，重名/墓碑名 409"},
			{"method": "PUT", "path": "/admin/accounts/{name}", "description": "改凭据/停启用/元数据 {token?,credentials_file?,credentials_content?,disabled?,priority?,max_rpm?,notes?} 指针语义；file 与 content 互斥；config 名首写自动建覆盖行；tombstoned 409 须先 restore"},
			...
			{"method": "POST", "path": "/admin/accounts/{name}/test", "description": "凭据连通性探测：恒 200 {ok,latency_ms,user?,plan?,error?}；成功顺带配额信号回灌+清冷却；名不在生效集/tombstoned/凭据不可解 404"},
