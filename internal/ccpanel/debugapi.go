...
func (h *Handler) adminAPIIndex(w http.ResponseWriter, r *http.Request) {
	respondOK(w, map[string]any{
		...
		"endpoints": []map[string]string{
			...
			{"method": "GET", "path": "/admin/auth-tokens?range=", "description": "下游令牌表 + range 内时间窗聚合统计（覆盖累计字段）；另投 master_key 元信息（key_hash 前缀）"},
			{"method": "POST", "path": "/admin/auth-tokens", "description": "创建下游令牌（并发槽/费用窗口/模型白名单），明文仅此一次返回"},
			{"method": "PUT", "path": "/admin/auth-tokens/{id}", "description": "更新令牌（启用/限额/白名单等）"},
