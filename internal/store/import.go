...
func (s *Store) importIndex(ctx context.Context, path string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			return s.withSourceTx(ctx, "index", func(q dbtx) error { return nil })
		}
		return err
	}
	defer func() { _ = f.Close() }()
	reader := bufio.NewReaderSize(f, 64*1024)
	// 整文件单事务会把唯一写连接按行数线性独占（数十万行实测数秒
	// 级）——分片提交在片间把连接让回，在役写者按到达序插队；中断
	// 重跑靠 dir 唯一索引幂等续导，已提交分片不丢。
	for eof := false; !eof; {
		err := writeTx(ctx, s.db.DB, "import:index", func(ctx context.Context, q dbtx) error {
			for pending := 0; ; {
				...
				if err == io.EOF {
					// 完成标记与末片同事务提交——标记缺席等于未导入完，
					// 重跑幂等续导。
					eof = true
					return markImported(ctx, q, "index")
				}
				if err != nil {
					return err
				}
