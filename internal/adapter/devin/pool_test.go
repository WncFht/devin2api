...
func TestPoolSessionBinding(t *testing.T) {
	...
	pool := newTestPool(t,
		Config{Name: "a", BaseURL: srv.URL, Model: "stub-model", Token: "tok-a"},
		Config{Name: "b", BaseURL: srv.URL, Model: "stub-model", Token: "tok-b"},
	)
	laneA := poolLaneByName(pool, "a")
	laneB := poolLaneByName(pool, "b")
	...
	if _, err := pool.ApplyConfigs([]Config{{Name: "a", BaseURL: srv.URL, Model: "stub-model", Token: "tok-a"}}); err != nil {
		t.Fatalf("ApplyConfigs: %v", err)
	}
