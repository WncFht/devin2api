...
- `logs/<dir>/03-devin-request.json`——adapter 实际发往上流的 protojson 请求（我们*实际*发了什么）。
- `logs/<dir>/04-devin-response.jsonl`——上游返回的 protojson 帧（它*实际*返回了什么）。
...
```sh
go run ./cmd/protocensus census                # 全部请求目录
go run ./cmd/protocensus census -max-dirs 500  # 只要最新 N 个目录
```

（装了 go-task 时 `task census [MAX_DIRS=N]` 包装同一命令。）默认读 `logs/`；仓库的 `logs/` 是指向平台 state 目录下 `logs/` 的软链，所以这查的是生产流量。
...
每个异常最多带 3 个 `examples`（请求目录名）；打开 `logs/<dir>/meta.json` 与被引用的阶段文件查上下文。
