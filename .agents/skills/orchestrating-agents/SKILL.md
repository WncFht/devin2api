...
工人拿到的是全新上下文：看不见你的对话。任务书是它的全部输入——必须**自包含**：目标、边界（明确的「不许做」清单）、你手里已有的证据（贴报错原文、点文件名）、约束、工具调用预算、`SUBTREE_BUDGET`/`DEPTH`、汇报格式。若允许它再派生，任务书里写清分配规则：子节点的 `SUBTREE_BUDGET` 之和 ≤ `k-1`，`DEPTH` 各加一。工人不需要读这份 skill——它该知道的一切由任务书携带，不由文件继承。
