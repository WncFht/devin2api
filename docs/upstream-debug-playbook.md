...
| 客户端报 429 且文案含 `rate limited by local gate`                                                                                              | devin-2api       | 本地速率闸门快败（令牌排队超 `gate_max_hold_seconds` 或冷却闩内非探针）         | 请求页按 `error_stage=rate_gate` 筛；系统页「速率闸门」看闩态；与上游真 429（`devin_connect`）分开归因                                |
