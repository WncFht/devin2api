...
| `incomplete envelope` / `unexpected EOF` / connection reset（含 `unavailable:`、`invalid_argument: protocol error:` 等 connect.Error 包装形态） | 传输             | 上游连接被截断（TUN/代理换路、上游连接回收），connect-go 统一包成 connect.Error                                             | 建立阶段重试 3 次；内容产出前断流整体重发一次（`meta.retry_attempts`/`index.retries` 落盘）；记 `devin_transport`。仍失败查连接路径   |
...
| 客户端报 429 且文案含 `rate limited by local gate`                                                                                              | devin-2api       | 本地速率闸门快败（令牌排队超 `gate_max_hold_seconds`、冷却闩内非探针，或上游续试重打时被闩拦）                              | 请求页按 `error_stage=rate_gate` 筛；系统页「速率闸门」看闩态；与上游真 429（`devin_connect`）分开归因                                |
