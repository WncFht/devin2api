...
| 现象                                                                                                                                            | 层               | 含义                                                                                                                        | 处理                                                                                                                                                            |
| ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HTTP 401                                                                                                                                        | devin-2api       | 凭据未命中令牌仓                                                                                                            | 查令牌仓（面板 auth-tokens 页 / `auth_tokens.json`）：无凭据但仓非空且无匿名通道行也走这条；`auth.api_key` 只是播种源，种子行被删后它不再通行                   |
...
| `incomplete envelope` / `unexpected EOF` / connection reset（含 `unavailable:`、`invalid_argument: protocol error:` 等 connect.Error 包装形态） | 传输             | 上游连接被截断（TUN/代理换路、上游连接回收），connect-go 统一包成 connect.Error                                             | 建立阶段最多尝试 3 次（`maxConnectAttempts`）；内容产出前断流整体重发一次（`meta.retry_attempts`/`index.retries` 落盘）；记 `devin_transport`。仍失败查连接路径 |
...
| 客户端报 503（`server_draining`）/ 429 `server is busy` / 401，但 index.jsonl 与请求页全是 200                                                  | devin-2api       | 管线前拒绝：排空 / 并发溢出 / 鉴权失败，未进门即被拒，不落调试目录                                                          | 面板系统页「本地拒绝」表查 reason+ 时刻；跨重启 `grep 'request rejected' logs/stderr.log`；503 必是排空（部署窗口内），429 看并发上限                           |
| 客户端报 401/403/429 且 index 行 `error_stage=token_limit`                                                                                      | devin-2api       | 下游令牌准入拒绝：并发槽 / RPM / 5h·日·周·月费用窗口 / 模型白名单 / 处理中令牌失效                                          | 有调试目录，看 `error.json` 的 message 区分哪条限额                                                                                                             |
...
| 流恰好 ~300s 整被掐、`error_stage=client_disconnected`                                                                                          | 客户端           | 客户端侧总超时（CC / ccload 各自的 request timeout）——代理没有 300s 这个限                                                  | 调客户端超时配置；代理侧时限只有排空上限（600s，仅退出窗口生效）与上游看门狗                                                                                    |
...
- **面板探活是真实计费上游调用**：`/admin/model-test`（ccpanel 探活）走真实 /v1 管线，`client_request_id=panel-probe` 落 index.jsonl——做 token 成本/命中率聚合时别误算进用户流量。
