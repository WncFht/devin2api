...
传输断裂类故障（envelope 截断/连接重置）可用 `cmd/upstreamstub` 本地复现：起桩监听后把测试实例 `devin.base_url` 指过去，`-scenario precontent|midcontent|recover` 分别对应「产出内容前断」「产出内容后断」「前 N 次断后恢复」，用于验证重试链路与 stage 归类。
