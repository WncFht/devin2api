...
| `unimplemented: ... capacity issues with this serving model ...`                                                                                | 上游             | serving 层按模型准入拒绝（硬币翻转式，非限流非请求错；2026-09-25 实测 ~8min episode 自愈、无惩罚机制）                                                                    | **本地已吸收**——pre-content 整体重开滴灌到打穿，客户端只见慢开流；拥塞期该模型在飞发送 cap=2（`congest.go`）。stderr 搜 `model congested` / `reopening stream: upstream serving model at capacity`；请求记录 04 看 `retry_attempt` 行数。终局透传按上游 502 归因                               |
...
| 客户端报 429 且文案含 `rate limited by local gate`                                                                                              | devin-2api       | 本地速率闸门快败（睡到下一可发窗口/滴灌槽的预计等待超 `gate_max_hold_seconds`，或上游续试重打过闸时被闩拦）                                                               | 请求页按 `error_stage=rate_gate` 筛；系统页「速率闸门」看闩态；与上游真 429（`devin_connect`）分开归因。CC 侧注意：未开 `CLAUDE_CODE_RETRY_WATCHDOG` 时 >60s Retry-After 抛 `api_request_retry_after_too_long` 零重试杀整轮（编排 agent 死因大头）；开了表现为睡到解闩续跑几十分钟——别误当卡死 |
...
| `preframe-error`                | 建流即拒：零数据帧、仅一条 EndStream 限流尾帧（`-reset-in` 秒数） | `devin_connect` 语义错误 + 速率闩（drip 探针 extended 路径）                                          |
...
与 `-scenario` 正交的 `-cache-mode none|content|trajectory` 让桩模拟上游前缀缓存记账并在 usage 帧回 cache_read——按 EPHEMERAL 断点边界匹配，content 跨轨迹按内容匹配、trajectory 只认同 trajectory_id 的既往请求。
