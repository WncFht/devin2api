...
`ModelFeatures` 能力位透出范围：`/v1/models` 只投影 5 个位——`supports_images`/`supports_tool_calls`/`supports_parallel_tool_calls`/`supports_thinking`/`preserve_thinking`（`modelCapabilities`，与 adapter `ListModels` 采集集相同）；`interleave_thinking` vendored proto 有字段但不投影，`supports_documents`/`summarize_thinking` 连 proto 都没有（仅上游 live payload 可见）。glm/kimi/grok/inkling/deepseek/nemotron 全系缺 `supports_parallel_tool_calls`。
