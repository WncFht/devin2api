...
>
> 证据分两级：**实测** = `cmd/probe` 或代理链路对真实上游打靶；**静态** = CLI 二进制 strings / 抓包 / proto bundle 分析。冲突时以实测为准。按日期排列的原始探测记录在 `archive/`（`2026-09-12-upstream-live-probes.md` 是实测档案、`2026-09-12-upstream-gaps.md` 是静态字段清单），本文吸收其结论。
>
