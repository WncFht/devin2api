...
>
> 2026-09-24 全量复测（configs/status×4/misc/registry/assign/chat/hist×4/replay×3/edge×25/toolchan/websearch 全绿）：**唯一实质变化是拒绝文案 mask 化**——历史形状类 `invalid_argument`/`unknown` 拒绝（分组 call→result、孤儿 TOOL 结果、未知 source）不再回 `an internal error occurred`，改披 `The third-party model provider is experiencing issues … try later` 瞬时模板（code 不变、6 帧后死/0 帧死的行为不变）；error 分类侧按 code 判不受文案影响。其余变动：`GetEmbeddings` 由 `unavailable` 改 `failed_precondition`（语义同：不对 token 开放）；`GetModelProviders` 13 家（+Meta，wire 值 13 在 vendored 绑定里无名）；目录 209→247 条（新增 gpt-5.6/claude-5.5·4.6/kimi-k2.7/nemotron-3-ultra/deepseek-v4-pro/gemini-3.x/gpt-5.3-codex 等族）。注意 `probe assign`/`chat -resolve` 对非 router uid（如 swe-2-max）报 `invalid_argument` 是**设计内行为**不是协议坏——swe-2-max 从来不是 router，真 router（subagent-default 等）照常签发 JWE。

## 探测工具

`cmd/probe`：直连 `server.codeium.com` 的 Connect-RPC 实验工具，复用 `outputs/devin-proto-go` 生成绑定，token 与 `config.yaml` 同账号。

- `chat`：逐字段打靶，flag 覆盖 `-model` `-max-tokens` `-temperature` `-top-p` `-top-k` `-stop-pattern` `-num-completions` `-system` `-prompt-id` `-provider-source` `-planner-mode` `-request-type` `-tool-choice` `-disable-parallel` `-custom-tool` `-raw-schema` `-tool-extras` `-images` `-doc-file`/`-doc-mime`/`-doc-url`/`-doc-history` `-vid-file`/`-vid-mime`/`-vid-url`/`-vid-history` `-rf-type`/`-rf-schema` `-prompt-cache-key` `-language` `-chat-model-name` `-meta-extras` `-no-fingerprint` `-no-ids` `-trajectory-id` `-step-index` `-cascade-id` `-assign-jwt` `-internal-model` `-resolve`/`-router` 等。
...
| `videos`（#20，`VideoData{base64_data/mime_type/url}`，3.10.31 新增；无 filename 字段） | 真实视频通道（2026-09-19 实测，已接线）：**per-model 按 `modelFeatures.supports_video`(#27) 门控**——本账号 209 uid 里仅 `kimi-k3-{low,high,max}` 与 `glm-5-3-flash-{low,high,max}` 6 个置位（`supports_video_urls`(#28) 与之恒共线）；swe-2-max → `invalid_argument: model "swe-2-max" does not support video inputs`；kimi-k3-high 接受 base64 mp4 且模型逐帧复述画面（**上游只送视觉轨——模型自述 "no audio data accessible"**）；**url 与 mime_type 互斥**（`"video url must not set a mime_type"`，同文档制）；url-only 形态有效（模型自取并描述远端 mp4）；**历史轮 videos 照常消费**（文档制，无图片的「仅当前轮」限制） |
...
| `response_format_type`(#18)/`response_format_json_schema`(#19) | **AI gateway 专属，CASCADE 不可达**（2026-09-19 实测）：CASCADE 下 → `invalid_argument: response_format "json_object" is only supported for AI gateway requests`；换 `-request-type AI_GATEWAY`（3.10.31 新增枚举）→ `permission_denied`（本 token 无 AI gateway 权限）。**adapter 故意不接**——转发会把客户端的 response_format 请求整单打回，维持现状按 Dropped 丢弃更安全 |
...
| RPC                                                                                      | 实测                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GetCliModelConfigs`                                                                     | 模型表 209 条（cascade 版 210，差一条 legacy `MODEL_CHAT_GPT_4O_2024_08_06`）；对比 09-13 快照新增 18 个 uid、无删除：`gpt-5-3-codex-{low,medium,high,xhigh}[-priority]`×8、`gpt-5-4-{none,low,medium,high,xhigh}[-priority]`×10。多 `subagent_default_model_uid`（=`subagent-default`）与 `default_override_model_config`（=`{swe-2-high, swe-2-high-09102026}`）；本账号零 router、`inference_config`/`smart_friend_model_uid` 全空。**2026-09-24:247 条**（新增 gpt-5.6-luna/terra、claude-opus-5.5、claude-sonnet-4.6 系、kimi-k2.6/2.7、nemotron-3-ultra、deepseek-v4-pro、gemini-3.0/3.1 等族），`is_model_router` 依旧全不下发 |
...
| `GetModelProviders`                                                                      | 12 家（xAI/DeepSeek/Qwen/NVIDIA/ThinkingMachines/Windsurf/OpenAI/Google/Moonshot/Z.ai/MiniMax/Anthropic）；2026-09-24 复测 13 家——+Meta（wire 值 13，vendored 绑定尚无该枚举名，渲染为裸数字）                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `GetStatus`/`GetConfig`                                                                  | 空响应                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `GetCommandModelConfigs`                                                                 | 6 条 legacy enum uid                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `GetEmbeddings`                                                                          | `unavailable`——不对该 token 开放（2026-09-24 复测改报 `failed_precondition`，语义同）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
`ModelFeatures` 能力位透出范围：`/v1/models` 投影 6 个位——`supports_images`/`supports_documents`/`supports_tool_calls`/`supports_parallel_tool_calls`/`supports_thinking`/`preserve_thinking`（`modelCapabilities`，与 adapter `ListModels` 采集集相同）；3.10.31 descriptor 刷新后 `supports_documents`(#29)/`supports_document_urls`(#30)/`supports_video`(#27)/`supports_video_urls`(#28)/`summarize_thinking`(#31) 都进了 vendored proto——documents/video 四位已投影（`/v1/models` + ccpanel upstream 视图），`supports_documents`/`supports_video` 还驱动 adapter 的本地门控（无对应能力模型 + 对应内容 → `invalid_argument`，镜像上游文案）；`interleave_thinking`/summarize 位不投影。glm/kimi/grok/inkling/deepseek/nemotron 全系缺 `supports_parallel_tool_calls`。
...
- `ChatMessagePrompt.videos`(#20) 视频通道同制（2026-09-19 实测）：kimi-k3-high 对 base64 mp4 逐帧复述画面内容，上游只送视觉轨（模型自述无音频数据）；`url`/`mime_type` 互斥与历史轮可读两条规则照搬文档；门控面更窄——本账号目录仅 kimi-k3 与 glm-5-3-flash 两系 6 个 uid 置 `supports_video`。
