...
- `rerun -file <03-devin-request.json>`：整包回放调试录制的 wire 请求（先用 `writefile()` 从 `debug_files` 导出，手法见 AGENTS.md「服务排障」节）。回放会刷新 `metadata.api_key` 为当前 token 并重铸 `execution_id`——非严格逐字节一致；同 body 6/6 复放被接受。大历史回放很慢：706KB 真实 Codex 会话在 probe 自带 300s ctx 内流不完（上游接受但不在窗口内收尾）。
