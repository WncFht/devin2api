...
钉选判定在脚本内用 sha256sum 复刻：`affinity = sha256(user)[0:16] hex`，rendezvous 分 `sha256(affinity|name)` 升序——与 `SessionAffinityKey`/`rankLanes` 同种子同序（完整排序六档：绑定命中未让位 → 在飞钉选 → 健康桶 → priority 降序 → 加权 HRW 键（weight 按 expectedWait/TTFB 中位数推出）→ 分数字节序兜底，脚本两号同 priority、无绑定无在飞且等权时即退化为分数序），因此脚本能预知每个会话键钉到哪条 lane，并从候选键里各挑 3 个钉到 bad / good。亲和键先取请求头链（`X-Claude-Code-Session-Id` → `X-Session-ID` → `X-Session-Affinity` → `X-Conversation-Id` → `X-Thread-Id`），脚本不传头时回落 body 的 `user` 字段。
