...
`deploy-linux.sh` 推生产实例后按本清单走一遍，确认版本切换完成且面板无回归。命令在 archbox 本机执行；生产实例监听 `:3033`，本机用 `http://127.0.0.1:3033`、其它机器用 tailscale 地址 `http://100.121.76.120:3033`（旧地址 `100.105.212.52:3003` 由 fht-mba 的 forwarder shim 转发，同样可达）。

## 步骤

1. 部署：`scripts/deploy-linux.sh`（构建工作树现状并部署）。
2. 确认版本：`curl -s http://127.0.0.1:3033/healthz`，`version` 字段等于刚部署的版本才算切换完成——排空期旧进程仍以旧版本应答，脚本退出或首个 200 都不代表切完。同一信息也可看 `/admin/status`。
