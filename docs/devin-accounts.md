...
账号的权威集是两层叠加：config.yaml 的 `devin.accounts` 是声明基座（永不落库、不被播种），面板写操作落 `upstream_accounts` 表成行。生效集 = config 声明 ∪ 活行 − 墓碑；同名时行逐字段覆盖声明值（行 `token`/`credentials_file` 非空即用行值），`disabled` 恒取行值。`/admin/accounts` 把每条生效项标 `source`：`config`（声明项，可有覆盖行）、`panel`（仅面板行）、`tombstoned`（面板删掉的 config 名——`deleted=1` 行压住声明，`POST .../restore` 复活）；config 撤名后残留的死墓碑不进列表，由下次生效集重推顺带 GC。面板改 config 名生成覆盖行（`has_override:true`，面板值恒赢、改空串回落声明值）；`disabled:true` 从 lane 集摘除（lane 异步排空 Close、在途流跑完），身份与闸门/配额历史保留——停用不是删除。

面板端点一览：

| 端点                            | 行为                                                                                                                                                                                                                                             |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /admin/accounts`          | 建号。`verify:true` 先以 `GetUserStatus` 探测凭据再建行；`credentials_content` 粘贴 credentials.toml 全文，校验可解后落盘 `<stateDir>/account-credentials/<name>.toml` 0600，行内按 credentials_file 口径存绝对路径                              |
| `PUT /admin/accounts/{name}`    | 改凭据/停启用/priority/max_rpm/notes。指针字段，显式空串=清覆盖；`credentials_content` 显式空串清 credentials_file 覆盖                                                                                                                          |
| `DELETE /admin/accounts/{name}` | config 名 → 墓碑；panel 名 → 物理删                                                                                                                                                                                                              |
| `POST .../restore`              | 复活墓碑                                                                                                                                                                                                                                         |
| `POST .../clear-cooldown`       | 清池侧两档冷却                                                                                                                                                                                                                                   |
| `POST .../quota/refresh`        | 单号即采配额                                                                                                                                                                                                                                     |
| `POST .../test`                 | 凭据连通性探测：恒 200 回 `{ok,latency_ms,user?,plan?,error?}`，成功顺带把配额信号回灌池侧并清冷却；名不在生效集/墓碑/凭据不可解 404                                                                                                             |
| `GET .../cli-credentials`       | CLI 凭据发现链探针，只报存在性不回内容                                                                                                                                                                                                           |
| `GET .../export`                | 生效集非墓碑条目整批导出：默认 yaml 下载、`?format=json` 出 JSON；credentials_file 型账号把文件内容内联成 `credentials_content`，跨机迁移不依赖原路径，disabled/notes 随行导出。**响应含全部凭据明文（token/api_key/内联文件内容），勿对外分享** |
| `POST .../import`               | body 是 `{accounts:[...]}` 或裸账号列表，yaml/json 皆可，形状同 export——按名 upsert：每条是该名期望全态，新名建行、config 名建覆盖行、墓碑名写 `deleted=0` 即复活；任一非法即整批 400 不落库，重推失败逐行回滚                                   |
