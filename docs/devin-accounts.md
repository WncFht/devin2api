...
亲和键的取值按显式优先链走：请求头链 `X-Claude-Code-Session-Id` → `X-Session-ID` → `X-Session-Affinity` → `X-Conversation-Id` → `X-Thread-Id` 取首个非空（头是调用方的显式意图，恒赢于 body 提取；`X-Client-Request-Id` 每轮都变、`X-Claude-Code-Agent-Id` 应共享父会话 lane，两者刻意不采）；头全空回落 body 提取——CC `metadata.user_id`、Codex `prompt_cache_key`，再空回退 system 头 4KB + 首条消息文本头 1KB 的内容哈希。亲和键与 trajectory/cascade ID 同种子，故 trajectory/cascade 派生、warm 谱系、assignment 三个命名空间随会话整体钉在同号上。

选号排序分四层：已绑定会话命中绑定 lane 恒第一（绑定是滑动 TTL，命中即续期，`devin.session_affinity_ttl_seconds` 控制时长，默认 3600 秒；绑定只在内存，重启后按同一条排序规则自然重钉）；未绑定时先按健康分桶（健康 → 配额低 → 冷却中，桶号越小越前），桶内按 `priority` 降序，同级再做 rendezvous hashing（`sha256(亲和键|lane名)` 排序取首）。成功开流的 lane 拿走该会话的绑定；lane 被摘出（disabled/墓碑/撤名）时其全部绑定随 lane 一起清。
...
`logs` 表每行带 `account`（产出终局结果的 lane 名——成功开流、终审拒绝或换号穷尽时的最后一号都算，凡触达 lane 的请求恒有值）与 `account_switches`（被试过又放弃的 lane 数，0 即首号出终局）；同 dir 的 `meta.json` 另带 `pool_candidates`（选号时刻的候选序快照：`name`/`healthy`/`bound`/`reason`——`bound` 即亲和绑定命中、`quota_low` 即配额降权落尾），`/admin/logs?account=<name>` 与逐号聚合按 `COALESCE(NULLIF(account,''),'default')` 折叠——`default` 参数命中号池前时代的空串行与真名 default 行两群。同 dir 的 `meta.json` 带 `upstream_account`（同口径）与 `upstream_attempts`（被放弃 lane 的有序尝试：account/elapsed_ms/code/message）——failover 救回的请求没有 error.json，这份尝试表是换号归因的唯一痕迹。多 lane 模式下 `04-devin-response.jsonl` 内插 `account_attempt` 分界行：各 lane 的上游帧续写同一流，分界行标明一段帧属于哪号。`03-devin-request` 词干的发送序号同样跨 lane 共享——failover 后新 lane 的首发续占 `attemptN` 分片而不覆写基座，各 lane 的 wire 体按发送序各存一份。`quota_samples` 表每号独立成行、带 `account` 列；`GET /admin/quota` 返回 `accounts` 组按名给各自曲线与耗尽预测 + `user` 身份快照（采样顺带取回的 name/email/plan_name，内存态、重启后首个采样点前缺席），顶层 points/daily/weekly 镜像最新采样（freshest）的账号序列保持后兼容。`GET /admin/runtime-metrics` 的 `accounts` 组按名给每号 `gate`、`warm` 与 `lane` 快照——`lane` 是池侧状态：`healthy`（选中判定近似快照）、`bound_sessions`（当前亲和绑定数）、`auth_cooldown_until`/`unhealthy_until`（两档冷却截止，凭据换新可提前解禁，簿记持久化在 `runtime_state` 的 `poolcool:<name>` 键、重启恢复）与 `last_failure_at/code/message`（最近一次换号失败归因）；顶层 `gate`/`warm` 仍是首 lane 快照。stderr 在换号时打 `devin account lane failed, failing over` 告警行（带 account 与 error）。`GET /admin/accounts` 是上述数据源的聚合视图：逐号给身份（source/disabled/凭据种类/token_sha·api_key_sha 指纹——明文永不出端点）、lane/gate/warm 快照、在途数与配额摘要；disabled/tombstoned 项 lane/gate/warm 为 null 但 inflight 仍报实际值。
