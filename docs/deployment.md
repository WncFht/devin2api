...
三个 deploy 脚本（macOS/Linux 共用 `scripts/deploy/lib-deploy.sh`）参数语义一致：`--release <tag|latest>` 装预编译二进制（sha256 校验）、`--no-restart` 只替换不重启、`--check` 对比 已安装/运行中/最新 release 版本、`--uninstall` 停用并移除服务与二进制（保留 config/logs）。服务未安装时首装自动生成服务定义并拉起；`config.yaml` 缺失时从 `config.example.yaml` 生成（随机 `dashboard.password`，tty 下提示粘贴 token；下游 /v1 令牌不入配置，到面板 /web/tokens.html 创建）。开工前的 preflight 拦截 sudo、缺依赖、占位 token、端口冲突、生效 config 里死引用的 `credentials_file`（9-18 断流根因；加载期现已改判该 lane 降级带病服役而非拒载，带病起跑照样拦下）；`/healthz` 版本对上后再打一发 `/v1/models` 验证上游鉴权。最小安装路径：免 clone 免 root 走 `scripts/install.sh` 一行（`curl -sSL .../install.sh | bash`——薄引导层按目标 tag 拉 deploy 管线到临时 staging，按 uname 分派 Linux→deploy-linux.sh / macOS→deploy.sh，install/upgrade/rollback/status/list-versions/uninstall 同语义）；要从检出驱动时 clone 仓库 → `deploy*.sh --release latest`。
...
- **面板自更新**：托管实例（systemd `--user` / launchd）在 `/web/settings.html` 有「版本更新」卡——检查更新 → 下载 GitHub release → sha256 校验 → 瞬态编排进程（Linux 走 `systemd-run --user` 瞬态 unit 逃出 `ProtectSystem=strict` 挂载禁闭，macOS 走 setsid 脱离 launchd 作业组）三段换名换二进制（当前→`.backup`、新版→当前）→ 跨 REUSEPORT 交接重启 unit——全程零断连，进度事实源是 `runtime_state` 表的 `update:status` 键，旧/编排/新实例接力写同一行，面板轮询无缝跨过进程切换；新实例 boot finalize 落 `done` 后编排进程自我终结。回滚不下载，`POST /admin/update/rollback` 对换名留下的 `.backup` 旧二进制做对称换回。端点族：`POST /admin/update`（body `{"tag"}` 可省取最新）、`POST /admin/update/check`、`GET /admin/update/status`、`POST /admin/update/rollback`（均 admin Bearer；在途冲突 409、无 `.backup` 404、坏 tag 400）。非托管形态（Windows 裸进程、手动二进制、容器）整族 501——升级仍走 deploy 脚本或换新镜像。残留的 `devin-2api-selfupdate-*` 瞬态 unit 与失管在途记录会被回收（活录驱动 pid 是 devin-2api 进程才收 SIGTERM；feature 前二进制能当编排但永不驱动，遗留 bound orphan 一并清）。
