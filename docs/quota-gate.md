...
| `reject_quota` / `reject_bg_reserve` / `reject_latch` / `reject_yield` | 按成因分列的快败数：排队预算耗尽（桶满/死区睡到下一窗口将超预算）、bg 让路（预留/爬坡）、闩内快败、让位（预计排队超长且兄弟 lane 能更快放行时提前拒，交给 failover；迁移 0012 起）；`reject_hold` 列已退役（hold 词并入 quota），恒为 0 |
