...
## 五点六、17:50 因果验证：A/B 回放 + 生产前后对比

`probe rerun`（新子命令）直接回放落盘的 `03-devin-request.json`，再用脚本把拆分历史变换为合并形态做对照。三次真实中途停止的请求各采样：
...
同一段历史、同一时刻、同一账号，唯一变量是 wire 形态——拆分编码把「宣告文本后采到 EOS」的概率从基线抬到 ~50%，合并后归零。这是因果证据，不只是相关性。

生产侧同步佐证：拆分时代（当日 480 请求）46 次 STOP_PATTERN 里约 15 次是宣告式中途停止；合并部署后 64 请求中 5 次 STOP_PATTERN 全部是完整最终回答（另 2 次为人工测试请求），**0 次宣告式停止**。生产样本量还小，但方向一致。

排除的其他嫌疑：

- **signature/outputId/thinkingRedacted 回放**：真实抓包从不带这些字段，但 merged-nosig 变体回放同样 6/6 FUNCTION_CALL——签名不是停止诱因。保留回放是因为 fedd2b9 的签名矩阵实验证明部分模型路径需要（type 配对强制）。
- **messageId 稳定性**：抓包 05→06 同消息 messageId 全部变化——真实客户端也是每请求随机，与我们一致。
- **trajectoryReference.stepIndex**：真实客户端携带（会话内单调计数，如 capture06 stepIndex=6），我们没发。这是残留的形态差距，但属轨迹记账字段不进渲染上下文，与停止无关联证据；忠实还原需要会话级计数状态，暂记为已知差异。
