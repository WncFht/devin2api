...
| 字段                                                                                   | 现状   | 判断                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tool_choice` (#12)                                                                    | 未映射 | 客户端 `tool_choice`（OpenAI `auto`/`required`/`none`/指定工具、Anthropic `auto`/`any`/`tool`/`none`）被静默丢弃。上游 `ExaChatPb_ChatToolChoice` 是 `oneof { option_name \| tool_name }`，可映射。CLI 自己不传（strings 无实证），但 OpenAI 客户端显式 `tool_choice` 时会丢语义——建议映射 `auto`→缺省、`none`→`option_name="none"`（需实测取值）、指定名→`tool_name`。 |
| `disable_parallel_tool_calls` (#11)                                                    | 未映射 | OpenAI `parallel_tool_calls=false` 被丢弃。上游布尔值直译即可。                                                                                                                                                                                                                                                                                                         |
...
| `request_id` (#17)                                                                                                                                                  | 没读           | 上游请求 id，排障时有用。**建议**：写到响应 `ResponseID` 旁边或日志里。                                                                                                                                    |
...
| `usage.provider_refusal` (#12 inside ModelUsageStats)                                                                                                               | 没读           | 上游拒绝信号——当 stop_reason 不是 CONTENT_FILTER 但 usage.provider_refusal=true 时，可作判据。**建议**：在 `finish` 时检查一次，provider_refusal=true 时把 fail 的 error message 写明 "provider refused"。 |
...
**含义**：`ListModels` 现在只透出 `supports_images`，可以把 `supports_tool_calls`/`supports_parallel_tool_calls`/`supports_thinking`/`preserve_thinking` 也透出去——让上层（ccload/客户端）在发请求前知道模型能不能并行调/带 thinking。
...
**含义**：`ContextTooLong` 和 `PayloadTooLarge` 是独立错误（不是 `invalid_argument`）。我们目前把所有非瞬时报错原样透传 message，客户端看到的 code 是 `permission_denied`/`invalid_argument` 的包装——如果上游真的返回 `ContextTooLong`，我们目前 map 不到 `invalid_request_error` 之类，会落到通用 `server_error`。**建议**：观察一次 `ResourceExhausted`/`out_of_range` 是否就是 ContextTooLong 的包装；必要时在错误归一表加 `context_too_long`/`payload_too_large` 条目。

### 8. `subagent_default_model_uid`（GetCliModelConfigsResponse #4）

CLI 用 GetCliModelConfigs 拿模型表 + `subagent_default_model_uid` 决定 sidekick/subagent 缺省模型。我们 `ListModels` 用的是 `GetCascadeModelConfigs`（只返回 client_model_configs + sorts + default_override_model_config），**没有** `subagent_default_model_uid`。若要支持代理侧 sidekick 调度，需要切到 `GetCliModelConfigs`。
...
3. **`tool_arg_leak_recovery` 同款防御**：CLI 实测发现 swe 模型偶尔把 native XML 调用语法（`<parameter name="...">...</parameter>` 或 `<antml:parameter ...>`) **混进 `arguments_json` 字符串**——CLI 用正则 `<(?:antml:)?parameter\s+name="([A-Za-z_][\w-]*)">` 检测并把泄漏的 XML 提取出来修复 args（strings: "repaired XML tool-call syntax leaked into tool arguments"）。我们 decoder 同样只做 JSON 拼接，遇到这种泄漏会产出坏 JSON 被兜底成 `{}`。建议在 `complete` 时检测 `arguments_json` 是否含 `<parameter`/`<antml:parameter` 并尝试把 XML 参数解回 JSON；失败时至少把原文给客户端而不是吞掉。
